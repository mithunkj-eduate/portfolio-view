...
function useProffionPortfolio() {
    ...
    const getUser = async () => {
      try {
        const res = await axios.get(
          `${apiBaseUrl}/api/professionalPortfolio/${id}`,
        );
        if (res && res.data) {
          setData(res.data);
          dispatch({
  ...
  const result = id === "kiran-farmer" ? farmerData : data;

  if (result) return result;
}

export default useProffionPortfolio;
